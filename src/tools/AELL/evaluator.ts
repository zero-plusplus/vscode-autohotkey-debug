...
import { BinaryExpressionNode, NumberLiteralNode, SyntaxKind } from '../../types/tools/AELL/common.types';
import { EvalFunc } from '../../types/tools/AELL/evaluator.types';
import { createAELLParser } from './parser';

const createEvalFuncMap = <Value, Evaluated extends Promise<Value>, Func extends EvalFunc<Evaluated>, FuncMap extends Record<SyntaxKind, Func>>(overrides?: FuncMap): FuncMap => {
  const funcMap = {
...
export const createEvaluator = <Value, Evaluated extends Promise<Value>, Func extends EvalFunc<Evaluated>, FuncMap extends Record<SyntaxKind, Func>>(version: AutoHotkeyVersion | ParsedAutoHotkeyVersion, overrides?: FuncMap) => {
  const evalFuncMap = createEvalFuncMap(overrides);
