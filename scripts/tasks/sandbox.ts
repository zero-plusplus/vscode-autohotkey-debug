...
export const startSandBox = async(): Promise<void> => {
  ...
  if (directoryExists(wsbDirPath)) {
    await fs.rmdir(wsbDirPath, { recursive: true });
  }
  await fs.mkdir(wsbDirPath, { recursive: true });
  ...
  await fs.cp(`${projectRootDir}/${sampleDirName}`, `${wsbDirPath}/${sampleDirName}`, { recursive: true });
